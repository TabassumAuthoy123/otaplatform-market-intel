  ...
  return (
    <div className="space-y-8">
      ...
      <Panel title="Last 10 days" sub="Sales, profit and closing cash per day">
        <Table head={['Date', 'Inv', 'Sales', 'Cost', 'Gross profit', 'Expenses', 'Net', 'Cash closing']} right={[1, 2, 3, 4, 5, 6, 7]}>
          {days.map((d) => (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel title="Sales by service" sub="Where the revenue comes from">
          <div className="px-5 py-3">
            {byService.map((r) => (
              <Bar key={r.service.id} value={r.sales} max={maxSvc} label={r.service.name} amount={`${moneyShort(r.sales, sym)} · ${moneyShort(r.profit, sym)} profit`} />
            ))}
          </div>
        </Panel>
