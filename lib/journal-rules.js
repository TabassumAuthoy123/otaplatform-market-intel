...
function validateVoucher(book, draft, isLocked) {
  ...
  /**
   * THE OPENING BALANCE IS NOT AN ACCOUNT YOU POST TO.
   *
   * EQUITY_OPENING is derived. buildJournal computes the whole of it, every render, from two
   * settings fields — company.openingCash and each bank's openingBalance — and credits it as
   * the counterweight to the cash and bank debits it raises from the same two. There is no
   * state behind it to correct.
   *
   * WHY THIS IS REFUSED RATHER THAN LEFT TO JUDGEMENT
   *
   * It is the first instinct of anyone who has closed a year before: bring last year in as an
   * opening entry. This product does not carry a year forward that way — the close records a
   * cut and posts nothing — so that voucher is not a correction, it is a duplicate of a
   * position the journal already raises.
   *
   * And it is invisible. Measured on the closed demo book, a balanced 20,609,100 voucher
   * dated 2026-07-01 crediting this account takes total assets from 2,39,24,824 to
   * 4,45,33,924 — and leaves reconciliation().clean true, both trial balances at zero, the
   * balance sheet difference at zero, the P&L bridge at zero and the year-end drift check
   * clean. Every guard in the product stays green while the balance sheet doubles, because a
   * balanced voucher is balanced whatever it says.
   *
   * So the refusal names the two fields that DO move it, because somebody reaching for this
   * account wants an opening position and there is a right place to set one.
   */
  const openingLines = lines.filter((l) => String(l.account || '').trim() === AC.EQUITY);
  if (openingLines.length) {
    errors.push(
      `Opening balances is derived, not posted. It is computed from Settings — the company's ` +
      `opening cash and each bank's opening balance — every time the journal is built, so a ` +
      `voucher against it would double the position rather than correct it, and every ` +
      `balance check in the product would still read clean. Change those two settings ` +
      `instead, or raise the entry against the account that actually moved.`
    );
  }
