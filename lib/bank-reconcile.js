...
function reconcile({
  ...
  /**
   * What manual journal vouchers have already put through this bank account in this
   * period, signed the way the ledger moves it.
   *
   * Derived by the caller from the journal rather than recorded anywhere, and that is
   * the point: it cannot be marked done without the entry actually existing.
   */
  postedToBank = 0,
  from,
  to,
  ...
  const groupCandidates = results.filter((r) => r.status === 'group_candidate');
  const matched = results.filter((r) => r.status === 'matched');

  /**
   * A line only counts as the bank's own when somebody SAYS it is.
   *
   * This used to treat every unmatched line as a bank charge, and that was the single
   * worst thing in the file. Two ordinary cases prove it:
   *
   *   A cheque written on 31 July and presented on 2 August matched nothing in August's
   *   book movements. It was declared a charge the book had never seen, the adjustment
   *   draft offered to post it — and the payment was already in the book from July.
   *
   *   Three customer cheques banked together arrive as one inward-clearing line. It
   *   matched nothing, so it became a "bank credit"; the three receipts stayed
   *   outstanding, so they became "deposits in transit". Both columns moved by the same
   *   amount, the difference stayed at zero, and the reconciliation reported itself
   *   perfect while offering to record the money a second time.
   *
   * Both are now handled at the source — carried-forward items are in the candidate pool
   * and an aggregated deposit is offered as a group — but neither fix can be complete,
   * because the next shape of this problem has not been met yet. So the general rule
   * stands underneath them: an unclassified line is EXCLUDED from the arithmetic, which
   * makes the difference non-zero and the omission visible, instead of included, which
   * makes it tie by coincidence and disappear.
   */
  const classified = (r) => (r.classification === 'bank_only');
  const unclassified = results.filter((r) => r.status === 'unmatched' && !classified(r));
  const bankOnly = results.filter((r) => r.status === 'unmatched' && classified(r));
  ...
  const bankCredits = bankOnly.filter((r) => r.line.direction === 'in').map((r) => r.line);
  const bankDebits = bankOnly.filter((r) => r.line.direction === 'out').map((r) => r.line);
   ...
   *
   * closing - opening must equal what the lines add up to. When the file carried a running
   * balance this holds by construction, so the check costs nothing and proves nothing —
   * but the moment a person types the closing figure, it is the only thing standing
   * between a typo and a reconciliation built on it. Both balances sit on the left of the
   * equation every other number here is compared against; an unchecked one propagates
   * into every verdict on the screen.
   */
  const linesNet = round2(
    results.reduce((t, r) => t + (r.line.direction === 'in' ? r.line.amount : -r.line.amount), 0)
  );
  const statementSpan = statementOpening === null || statementOpening === undefined
    ? null
    : round2(round2(statementClosing) - round2(statementOpening));
  const selfConsistent = statementSpan === null ? null : statementSpan === linesNet;

  const blockers = [];
  if (selfConsistent === false) {
    blockers.push(
      `The statement does not agree with itself: it opens at ${round2(statementOpening)} and closes at ${round2(statementClosing)}, a movement of ${statementSpan}, while its own lines add up to ${linesNet}. Nothing below can be trusted until that is resolved — check the balances and whether every line was captured.`
    );
  }
  if (unclassified.length) {
    const value = round2(unclassified.reduce((t, r) => t + (r.line.direction === 'in' ? r.line.amount : -r.line.amount), 0));
    blockers.push(
      `${unclassified.length} statement line(s) worth ${value} match nothing in the book and have not been classified. They are left OUT of the arithmetic below, which is why the difference is not zero: until somebody says whether each one is the bank's own charge or a book entry nobody has recorded, counting them either way would be a guess.`
    );
  }
  if (groupCandidates.length) {
    blockers.push(
      `${groupCandidates.length} statement line(s) look like several book entries banked together. Confirm the grouping and they match as a set; leave it and they stay outside the arithmetic.`
    );
  }
  if (ambiguous.length) {
    blockers.push(
  ...
  return {
    ...
    difference,
    openingGap,
    /** Whether the statement's own opening, closing and lines agree. Null when untestable. */
    selfConsistent,
    linesNet,
    statementSpan,
    /** Lines matching nothing that nobody has yet called a bank charge. Excluded above. */
    unclassified: unclassified.map((r) => r.line),
    unclassifiedTotal: round2(unclassified.reduce((t, r) => t + (r.line.direction === 'in' ? r.line.amount : -r.line.amount), 0)),
    groupCandidates,
    /**
     ...
     */
    /**
     * How many classified items are still waiting for a journal voucher.
     *
     * The first version counted every classified line and never came down, so a person
     * could classify four bank charges, post them correctly, come back, and be told the
     * period was not finished — with nothing on the screen to do about it. A feature that
     * asks for an action and then cannot see it happen is worse than one that never asked.
     *
     * Detected rather than recorded. A voucher that hits this bank account inside the
     * period moves the JOURNAL balance while leaving bankBook's voucher-derived closing
     * alone, so the two can be compared: once the journal has moved by exactly what the
     * classified items are worth, they are on the book. Nothing needs to be ticked off,
     * and nothing can be ticked off without the entry actually existing.
     *
     * A partial posting leaves this non-zero and the difference visible, which is right —
     * three charges recorded and one forgotten is precisely the state worth catching.
     */
    requiresPosting:
      round2(postedToBank) === round2(bankCreditsTotal - bankDebitsTotal)
        ? 0
        : bankCredits.length + bankDebits.length,
    postedToBank: round2(postedToBank),
    postingExpected: round2(bankCreditsTotal - bankDebitsTotal),
    ...
    counts: {
      ...
      groupCandidate: groupCandidates.length,
...
function adjustmentDraft(rec, bankAccountCode) {
  ...
  return {
    /**
     * The LATEST item's own date, not the period end.
     *
     * Dating every adjustment at the period end put a charge taken on the 3rd into the
     * 31st, which is wrong twice over: it moves the expense to the wrong day inside the
     * month, and if the period end sits in a closed period the whole voucher is refused
     * for a reason that has nothing to do with the items in it. The per-line dates are on
     * the memos, so an accountant splitting this into several vouchers has what they need.
     */
    date: lines.length
      ? lines.map((l) => String(l.memo).slice(0, 10)).filter((d) => /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(d)).sort().pop() || rec.to
      : rec.to,
