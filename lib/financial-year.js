...
/**
 * THE WINDOW OF THE YEAR BEING CLOSED. Not the same question as openWindow.
 *
 * openWindow answers "what should a report show by default". This answers "which dates belong
 * to the year about to be filed", which is the span from the day after the previous cut up to
 * the proposed one. `from` is null on a first close and that means the book's own beginning,
 * not the financial year start — same reason openYearStart is null rather than a date.
 *
 * IT EXISTS BECAUSE THE CLOSE COMPARED A YEAR AGAINST A LIFETIME.
 *
 * The close derives the year two ways and refuses to file a year the book cannot agree with
 * itself about. The journal side was bounded to this window by subtracting the previous cut;
 * the voucher side was not bounded at the bottom at all. They agreed on the first close of
 * this book for one reason only — there was no previous cut to subtract, so the year WAS the
 * lifetime — and would have disagreed on every close after it. Measured on the demo book for
 * a second close at 2027-06-30:
 *
 *   journal, this year          597,136
 *   vouchers, whole book        812,360
 *   ------------------------------------
 *   the gate refused by        -215,224   accusing the book of disagreeing with itself
 *
 * A permanent refusal, on a product whose whole point is that the year gets closed.
 */
function closingWindow(book, through) {
  const previous = lastClose(book);
  return { from: previous ? nextDay(previous.closedThrough) : null, to: through };
}
